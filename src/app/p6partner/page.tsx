...
import Heading5 from "@/components/p6partner/heading5/Heading5";
const PartnerList: React.FC = () => {
  ...
  return (
    <div>
      <Heading5 />
      <div className="mt-6 w-[100%] grid grid-cols-1  sm:grid-cols-1 md:grid-cols-5 gap-2 py-3 border-2 border-black">
        {Partner.map((item) => (
          <div
            key={item.id}
            className="flex  bg-white border rounded-lg p-2 mx-[1rem] shadow-md flex-shrink-0  border-2 border-black"
          >
            <div className="bg-white border rounded-lg p-4 shadow-md flex-shrink-0 w-auto ">
              <div className="flex justify-center mb-4">
                <img src={item.logoSrc} alt={`${item.name} logo`} className="h-12" />
              </div>
